...
descriptives_table <- s2_df %>%
    dplyr::filter(followed_VGinstructions == "Yes" & attention_all_correct == TRUE) %>%
...
# Analysis N
s2_analytic_N <- s2_df %>%
    dplyr::filter((attention_all_correct == TRUE) & (followed_VGinstructions == "Yes")) %>%
    nrow()
...
s2_race_multi <- s2_df %>%dplyr::summarise(across(starts_with("Race"), ~ is.na(.) %>% sum()))
```
...
parasocial_MC_txt <- parasocial_MC_txt %>%
    append(list(
        ...
        nonsurrogacy_noint = parasocial_MC_results_df %>%
            dplyr::filter(essay_condition_title == "Non-Social Surrogacy" &
                parasocial_MC_group == "No Interaction with NPC") %>%
dplyr::pull(percent_per_cond) %>% round(2),
...
dplyr::pull(percent_per_cond) %>% round(2),
        surrogacy_noint = parasocial_MC_results_df %>%
            dplyr::filter(essay_condition_title == "Social Surrogacy" &
                parasocial_MC_group == "No Interaction with NPC") %>%
dplyr::pull(percent_per_cond) %>% round(2),
...
manikin_ANOVA_DT <- manikin_mixed_ANOVAs %>%
    ...
    unnest(ANOVA) %>%
    dplyr::select(Variable:ges, -`p<.05`) %>%
...
manikin_ttests_DT <- manikin_mixed_ANOVAs %>%
    dplyr::select(Variable, pairTimed) %>%
    unnest(pairTimed)
