...
get_desc_db <- function(describe_by_factor_df,
  ...
  descriptives_long <- describe_by_factor_df %>% 
    ...
    mutate(variable = str_match(name, "^[^_].+(?=_)") %>% as.vector(),
...
get_APA_from_msd <- function(describe_by_factor_df,
  ...
  descriptives_long <- describe_by_factor_df %>% 
    ...
    mutate(variable = str_match(name, "^[^_].+(?=_)") %>% as.vector(),
           stat = str_match(name, "(?:[^_](?!_))+$") %>% as.vector())
  db <- descriptives_long %>%  pivot_wider(-name, values_from = value, names_from = stat) %>% 
...
get_correlations  <- function(target_df,
  ...
  df <- target_df %>% 
    summarise(across(!!vars, 
                     ~list(model95 = cor.test(pull(cur_data(), !!outcome), .x, conf.level = conf.level1),
                           model90 = cor.test(pull(cur_data(), !!outcome), .x, conf.level = conf.level2))))
  model_names <- df %>% pull(1) %>% names
  df <- df %>% 
  ...
  df <- df %>% 
    pivot_wider(id_cols = c("label", "estimate", "df"), names_from = c("Conf_Level"),
...
default_color_continuous <- scale_color_continuous(palette="Set2")
...
default_violin <- list(
  ...
  geom_errorbar(fun.data = mean_cl_normal,
                stat = "summary",
                width = .1,
                position = dodge_pos,
                color = "black", alpha = 1),
...
s1e_plot_across_time <- function(df_long, y, ylab = "", caption = "", title = ""){ 
  df_long %>% 
    ...
    ggplot(aes(x = time, y = {{y}}, color = paste(rejection, confederate_desire), 
...
s1_render_kable <- function(df, studykey = " "){
  ...
  pack_named_index <- df %>% filter(!is.na(intervals)) %>% select(labels, intervals) %>% deframe()
  ...
  if(needs_packing){
    returned_kable <- returned_kable %>% 
      pack_rows(index = pack_named_index,
...
describe_chi_htest <- function(x){
  ...
  apastring <- sprintf("X<sup>2</sup>(%i, _N_ = %s) = %.2f, \\emph{p} %s",
...
describe_anova_test <- function(x){x %>%
...
describe.r <- function (rc, ..., .add_CI = FALSE){
  ...
  if(.add_CI == TRUE){
    ...
    format.results(sprintf("\\emph{r}(%.0f) = %.2f, \\emph{p} %s, %s%%CI [%s, %s]", 
                           rc$parameter, rc$estimate, round.p(rc$p.value), 
...
get_cronbach_alphas <- function(reliabilities){
  ...
  cronbach <- map(reliabilities,
                  function(x){if(is.null(x)){NA}
                    x$cronbach.alpha}) 
  return(cronbach)
}
...
add_eqtest <- function(lm_df){
  outdf <- lm_df %>% 
    ...
    mutate(eq_label = case_when(eq_pred == "Rejected" ~ "",
                                eq_pred == "Accepted" ~ "*",
                                # Unicode 
...
evaluate_equivalence <- function(lower, upper, ROPE){
  ...
  if(lower_in_ROPE & upper_in_ROPE){return("Accepted")}
  if(!lower_in_ROPE & !upper_in_ROPE){return("Rejected")} 
...
get_n_pct <- function(df, ...){
  df %>% group_by(...) %>% summarise(n = n()) %>%
    ...
    nest_by(...) %>% ungroup() %>% # Ungroup to avoid cannot be recycled one error
...
plot_s2_moderation <- function(model){
  ...
  if("parasocial_MC_group" %in% terms_attr){
    ...
    geom_now <- list(scale_x_discrete(labels = s2mod_relabel),
  ...
  model %>% sjPlot::plot_model(show.values = TRUE, colors = "Set2",
...
plot_s3_mod_bymc <- function(model){
  ...
  terms_labels <- c("parasocial_MC_group" = "parasocial_MC_group",
                    "IOS" = "IOS",
                    "PSI" = "PSI",
  ...
  if("parasocial_MC_group" == current_moderator_label){
    # Geoms for the parasocial MC
    geom_now <- list(scale_x_discrete(labels = s3mod_relabel),
      labs(caption = paste("The reference group for the Parasocial Group is No Parasocial Relationship with NPC",
  ...
  p <- model %>% 
    sjPlot::plot_model(show.values = TRUE, colors = "Set2",
                       value.offset = .5, value.size	= 2.5) +
    geom_now
  return(p)
}
...
corstarsl <- function(x){ 
  ...
  R <- R %>% f.round(strip.lead.zeros = TRUE) %>% as.matrix(ncol = ncol(x))
...
get_slope_APA <- function(b, SE, df, ci.lower, ci.upper, b.digits = 2){
  sprintf(paste0("\\emph{B} = %.", 
         b.digits, "f, \\emph{SE} = %.",
         b.digits, "f, 95%%CI[%.2f, %.2f]"),
         b, SE, ci.lower, ci.upper) %>% format.results()
}
...
get_APA_from_emm <- function(emm){
  ...
  stats_df <- emm %>% as_tibble() %>%
    select(ends_with(".trend"):last_col())
  labels_df <- emm %>% as_tibble() %>% 
  ...
  # Deframe as list
  APA_output <- combined_df %>% select(1, APA) %>% 
    deframe_as_list()
}
