...
# add ronames
s1_measures_tbl <- s1_measures_tbl %>% dplyr::mutate(id = row_number())
...
pack_starts <- s1_measures_tbl %>%
    ...
    dplyr::mutate(Submeasure = row_number()) %>%
    dplyr::mutate(
    ...
    dplyr::mutate(labels = case_when(need_packing == TRUE ~ `Main Measure`, TRUE ~ " ")) %>%
...
s1_measures_tbl <- s1_measures_tbl %>%
    dplyr::mutate(labels = case_when(is.na(labels) ~ "", TRUE ~ labels))
...
s1a_cor_CIs <- s1a_cor_CIs %>%
    ...
    dplyr::mutate(eq_decision = case_when(
    ...
    dplyr::mutate(
        ...
        emm_time_acceptance_tidy = map(emm_time_acceptance, function(x) {
            x$contrasts %>%
                ...
                dplyr::mutate(acceptance = case_when(
...
s1d_df <- s1d_df %>%
    dplyr::mutate(confederate_desire = case_when(
...
s1e_NTS_APA <- s1e_NTS_APA %>%
    append(s1e_NTS_desc %>% select(label, APA) %>% tibble::deframe() %>% as.list())
