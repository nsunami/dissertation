...
s3_playtime <- s3_df_attentive %>%
    ...
    mutate(APA = to_msd(mean_playtime, sd_playtime)) %>%
    as.list()

# Getting participants per condition
s3_ntable <- s3_df %>%
    group_by(parasocial, social_world) %>%
    summarise(n = n()) %>%
    mutate(key = str_c(str_extract(parasocial, "^."), str_extract(social_world, "^.")))
s3_n_txt <- s3_ntable %>%
    ungroup() %>%
    select(key, n) %>%
    deframe() %>%
...
s3_attention_df <- s3_df %>%
    group_by(attention_all_correct) %>%
    summarise(n = n()) %>%
    deframe_as_list()
...
s3_att <- list(
    v1 = s3_df %>% filter(StartDate < s3_att_v2change_date) %>% nrow(),
    v2 = s3_df %>% filter(
...
s3_time_APA <- s3_time_msd %>%
    mutate(label = str_c(time, variable)) %>%
    select(label, APA) %>%
    deframe_as_list()
# APA
s3_manikins_time_list <- s3_manikins_time %>%
    mutate(APA_def = map(APA, function(x) {
        x %>%
            select(Effect, APA) %>%
            deframe_as_list()
    })) %>%
    select(outcome, APA_def) %>%
    deframe_as_list()
```
